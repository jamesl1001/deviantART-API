...
function getDeviations($url, $limit = null, $start = 0) {
    ...
    foreach($channel->item as $item) {
        ...
        $object = (object)array(
            ...
            "thumbS"        => ($item->children('media', true)->thumbnail) ? $item->children('media', true)->thumbnail[0]->attributes()->url : null,
            "thumbL"        => ($item->children('media', true)->thumbnail) ? $item->children('media', true)->thumbnail[1]->attributes()->url : null,
            "image"         => ($item->children('media', true)->content) ? $item->children('media', true)->content->attributes()->url : null,
